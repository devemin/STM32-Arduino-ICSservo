...
<br>（<b>EEPROM書き込みを行う set_EEPROM 関数のみ、現在はベータ版としておきます。バグやバッファオーバーラン等によるサーボ破損のリスクがゼロではないです。必要な方のみset_EEPROM関数をお使いください。詳しくは下の方に書きました。</b>）
<br>
...
# ●EEPROM書き込みを行う set_EEPROM 関数について（重要！）
...
<br>set_EEPROM 関数をご利用の方は、まず1か所（手先など）からお試し頂き、その後、他の場所に使ってみてください。万が一エラーの出た方は、エラーコードやコーディングとともにお教え頂ければ幸いです。

...
<br>（<b>EEPROM書き込みを行う set_EEPROM 関数のみ、現在はベータ版としておきます。サーボ移動などほかの関数は全て安全に使えますが、EEPROM書き込み処理だけは、バグやバッファオーバーラン等によるサーボ破損のリスクがゼロではないからです。必要な方のみコード内容をご理解の上、set_EEPROM関数をお使いください。詳しくは下の方に書きました。</b>）
<br>

...
# ●機能
・サーボ移動、脱力
<br>・全パラメータ読み書き（EEPROM含む）（ストレッチ、スピード、パンチ、デッドバンド、ダンピング、セイフタイマー、回転モード、PWM禁止、リバースモード、上下リミット、通信速度、温度制限、電流制限、レスポンス、オフセット、キャラスタリスティックチェンジのストレッチ1-3）
<br>・ID書き換え（通常コマンド、ホスト－サーボを１対１接続時）
<br>
<br>・<b>（独自）</b>サーボ脱力後に同位置で即動作（現在位置確認用）
<br>・<b>（独自）</b>ID書き換え（EEPROM書き替えにより、複数接続時でも可能）
<br>・<b>（独自）</b>ID指定によるサーボ存在確認の関数
<br>
<br>
# ●動作確認
